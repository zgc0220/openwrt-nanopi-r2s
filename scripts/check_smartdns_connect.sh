...
if [ ${SMARTDNS_ENABLED} -eq 1 ]; then
  while [ $tries -lt 5 ]; do
    ...
    NSLOOKUP=$(nslookup -port=${SMARTDNS_PORT} $ALIYUN 127.0.0.1 2>/dev/null | grep -v grep | grep 'Address:' | wc -l)
    if [ ${NSLOOKUP} -ne 0 ]; then
      echo $DATE check smartdns connect: OK >>$LOG
      exit 0
    else
      echo $DATE check $DNSPOD start >>$LOG
      NSLOOKUP=$(nslookup -port=${SMARTDNS_PORT} $DNSPOD 127.0.0.1 2>/dev/null | grep -v grep | grep 'Address:' | wc -l)
      if [ ${NSLOOKUP} -ne 0 ]; then
        echo $DATE check smartdns connect: OK >>$LOG
        exit 0
      else
